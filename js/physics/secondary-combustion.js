...
import {plumeWeight, PLUME_HEIGHT} from './plume.js';
import {updateBrickHeat} from './solver-base.js';
import {addBuoyancy} from './combined-buoyancy.js';
import {projectVelocity} from './conservation.js';
import {advectField} from './closed-scalar.js';
import {updateTracers} from './tracer-recycle.js';
...
export function reconcileSecondaryGeometry(previousSolid) {
  for (let i = 0; i < N; i++) {
...
export function applyFireAndRadiation(dt) {
...
export function secondaryZoneFactor(gx, gy, i) {
...
export function applySecondaryCombustion(dt) {
  let reactedTotal = 0;
  let reactedCells = 0;
...
export function updateSecondaryResidence(dt) {
  for (let i = 0; i < N; i++) {
    if (solid[i]) continue;
    const gx = i % NX;
...
export function applySecondaryAirPorts(dt) {
...
export function updateFlyAshEntrainment(dt) {
  for (let i = 0; i < N; i++) {
...
export function copyFluidBoundaries() {
  const freshen = i => {
    temperature[i] = AMBIENT_T;
    oxygen[i] = AMBIENT_O2;
    smoke[i] = 0;
...
export function ashMassRemaining() {
  let sum = 0;
  for (let i = 0; i < N; i++) {
    if (solid[i]) continue;
...
export function charMassRemaining() {
  let sum = 0;
  for (let i = 0; i < N; i++) {
    if (solid[i]) continue;
    sum += charResidue[i];
...
export function getSecondaryScalars() {
  return {
    get secondaryIndex() { return secondaryIndex; },
    get smokeOutIndex() { return smokeOutIndex; },
    get ashGeneratedTotal() { return ashGeneratedTotal; },
    get ashDepositedTotal() { return ashDepositedTotal; },
    get ashOutTotal() { return ashOutTotal; },
    get ashClearedTotal() { return ashClearedTotal; },
    get ashNumericalCorrectionTotal() { return ashNumericalCorrectionTotal; },
    get ashBurnedTotal() { return ashBurnedTotal; },
    get charGeneratedTotal() { return charGeneratedTotal; },
    get charBurnedTotal() { return charBurnedTotal; },
    get charOutTotal() { return charOutTotal; },
    get charClearedTotal() { return charClearedTotal; },
    get charNumericalCorrectionTotal() { return charNumericalCorrectionTotal; },
    get flyAshOutTotal() { return flyAshOutTotal; },
    get flyAshLiftedTotal() { return flyAshLiftedTotal; },
    get flyAshDepositedTotal() { return flyAshDepositedTotal; },
